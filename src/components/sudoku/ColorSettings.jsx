        ...
        >
          ...
          <div className="flex items-center justify-between p-3 sm:p-6 border-b border-slate-800">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <h3 className="text-base sm:text-xl font-semibold text-white">Color Settings</h3>
                <p className="text-xs sm:text-sm text-slate-400">Customize your theme</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          {/* Content */}
          <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto max-h-[calc(95vh-160px)] sm:max-h-[calc(90vh-180px)]">
            {/* Saved Presets */}
            {presets.length > 0 && (
              <div>
                <h4 className="text-xs sm:text-sm font-medium text-slate-300 mb-2 sm:mb-3">Saved Presets</h4>
                <div className="space-y-2">
                  {presets.map((preset) => (
                    <div
                      key={preset.id}
                      className="flex items-center gap-2 sm:gap-3 bg-slate-800 rounded-lg p-2 sm:p-3 hover:bg-slate-750 transition-colors"
                    >
                      <div className="flex gap-0.5 sm:gap-1">
                        {Object.entries(preset.colors).slice(0, 4).map(([key, color]) => (
                          <div
                            key={key}
                            className="w-4 h-4 sm:w-6 sm:h-6 rounded border border-slate-600"
                            style={{ backgroundColor: color }}
                          />
                        ))}
                      </div>
                      <span className="flex-1 text-sm sm:text-base text-white font-medium truncate">{preset.name}</span>
                      <button
            ...
            {/* Save Current Colors */}
            <div>
              <h4 className="text-xs sm:text-sm font-medium text-slate-300 mb-2 sm:mb-3">Save Current Theme</h4>
              {showSaveInput ? (
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    maxLength={20}
                  ...
                  <button
                    ...
                    className="px-2 sm:px-4 py-2 bg-slate-800 text-slate-300 text-xs sm:text-base rounded-lg hover:bg-slate-700"
                  >
                    Cancel
                  </button>
                </div>
                ...
                >
                  {presets.length >= 3 ? 'Max Presets (3/3)' : `Save Preset (${presets.length}/3)`}
                </button>
              )}
            </div>
            ...
            <div className="flex gap-1.5 sm:gap-2 flex-wrap">
              {Object.entries(sections).map(([key, section]) => (
                <button
                  ...
                  className={`
                    px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium text-xs sm:text-sm transition-all
                    ${activeSection === key
            ...
            <div className="bg-slate-800 rounded-xl p-3 sm:p-4">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <span className="text-xs sm:text-sm text-slate-300 font-medium">Preview</span>
                <span className="font-mono text-xs sm:text-sm text-slate-400">
                  {colors[activeSection]}
                </span>
              </div>
              <div className="flex items-center justify-center bg-slate-900 rounded-lg p-3 sm:p-4 min-h-[60px] sm:min-h-[80px]">
            ...
            {/* Preset Colors */}
            <div>
              <h4 className="text-xs sm:text-sm font-medium text-slate-300 mb-2 sm:mb-3">Preset Colors</h4>
            ...
            {/* Custom Hex Input */}
            <div>
              <h4 className="text-xs sm:text-sm font-medium text-slate-300 mb-2 sm:mb-3">Custom Color (Hex)</h4>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={customHex}
                  ...
                  placeholder="#RRGGBB"
                  className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base text-white font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                  maxLength={7}
                />
                <button
          ...
          {/* Footer */}
          <div className="flex items-center justify-between p-3 sm:p-6 border-t border-slate-800">
            <button
              onClick={handleReset}
              className="px-3 sm:px-4 py-2 text-xs sm:text-sm text-slate-400 hover:text-white transition-colors"
            >
              Reset to Defaults
            </button>
            <button
              onClick={onClose}
              className="px-4 sm:px-6 py-2 bg-blue-500 text-white rounded-lg text-sm sm:text-base font-medium hover:bg-blue-600 transition-colors"
            >
              Done
            </button>
          </div>
